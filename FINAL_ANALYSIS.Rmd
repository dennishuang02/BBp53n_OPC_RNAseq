...
diff.table.EZH2i$ENSEMBL <- rownames(diff.table.EZH2i)
...
Volcano Plot for glioma subtypes in BBp53nvsp53n
```{r}
library(ggplot2)
glioma_GS <- read.csv("Geneset/gliomaType_enrichments.csv", header = TRUE)

highlight_df <- diffExp %>%
    filter(SYMBOL %in% glioma_GS$Proneural)
highlight_label_df <- diffExp %>%
    filter(SYMBOL %in% c("Cnp", "Mag", "Myrf", "Mbp", "Sox8", "Erbb2", "Prmt5"))
...
#  geom_label_repel(data = highlight_label_df, aes(label = SYMBOL), color = "black", box.padding = 1, 
#                  point.padding = 0.5, segment.color = 'black', size = 5) +
  geom_vline(xintercept=c(-1, 1), col="black", alpha = 0.5, linewidth = 1, linetype = "twodash") +
  geom_hline(yintercept = -log10(0.01), col = "black", alpha = 0.5, linewidth = 1, linetype = "twodash") +
  #xlim(c(-4, 4)) +
  #ylim(c(0,10)) + 
  theme(legend.position = "none", 
        text=element_text(size=20), 
        ...
        panel.grid.major = element_blank(),
        axis.line = element_line(color = "black")) +
  ggtitle("Proneural - BB-p53n vs p53n OPC")
```
...
```{r}
highlight_df <- diffExp %>%
    filter(SYMBOL %in% glioma_GS$Classical)
...
#                  point.padding = 0.5, segment.color = 'black', size = 5) +
  ...
  theme(legend.position = "none", 
        ...
        panel.grid.major = element_blank(),
        axis.line = element_line(color = "black")) +
  ggtitle("Classical - BB-p53n vs p53n OPC")
```

```{r}
highlight_df <- diffExp %>%
    filter(SYMBOL %in% glioma_GS$Mesenchymal)
...
#                  point.padding = 0.5, segment.color = 'black', size = 5) +
  ...
  theme(legend.position = "none", 
        ...
        panel.grid.major = element_blank(),
        axis.line = element_line(color = "black")) +
  ggtitle("Mesenchymal - BB-p53n vs p53n OPC")
```
...
Calculate Z-scores
```{r}
counts_tpm_filtered <- counts_tpm[rowSums(counts_tpm) >= 10,]
